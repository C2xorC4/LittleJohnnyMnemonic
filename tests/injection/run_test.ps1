...
#
# Usage:
#   .\run_test.ps1 -Test <1-5>               # set up repo + run hook simulation
#   .\run_test.ps1 -Test <1-5> -Rollback     # remove payload from test repo
#   .\run_test.ps1 -Test <1-5> -Rename <tag> # move payload to results/ with tag
...
if ($List) {
    ...
    foreach ($n in 1..5) {
        ...
        $det = if ($d.ExpectedDetect) { "YES" } else { "NO -- gap test" }
        Write-Host ("      Expected detection: {0}" -f $det) -ForegroundColor DarkGray
...
if ($Test -lt 1 -or $Test -gt 5) {
    Write-Host "Usage: .\run_test.ps1 -Test <1-5> [-Rollback] [-Rename <tag>] [-List]" -ForegroundColor Red
    exit 1
}

$def       = $testDefs[$Test]
$repoDir   = "{0}_T{1}_{2}" -f $tempBase, $Test, $def.Name
...
$detExpect = if ($def.ExpectedDetect) { "YES" } else { "NO -- gap test" }
...
if ($detected -and $def.ExpectedDetect) {
    ...
    Write-Host "[DETECTION] PASS -- no warning, gap confirmed" -ForegroundColor Magenta
